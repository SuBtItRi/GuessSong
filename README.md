...
## Публикация в ветку `publish`

Команда делает:
1) запоминает текущую ветку, при необходимости делает `stash`  
2) переключается на `publish`, меняет версию (по флагу), запускает `yarn build`, коммитит `dist` и пушит в `origin/publish`  
3) возвращает обратно на исходную ветку

Использование:
- `yarn pub` (minor bump, например `0.0.0` -> `0.1.0`, коммит `v0.1`)
- `yarn pub --major` (major bump, например `0.5.0` -> `1.0.0`, коммит `v1.0`)
